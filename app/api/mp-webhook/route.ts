...
// Mercado Pago llama a esta URL (notification_url) cuando cambia el estado de un pago.
  ...
  try {
    ...
    if (info.status === "approved" && process.env.RESEND_API_KEY && process.env.NOTIFY_EMAIL) {
      ...
      await resend.emails.send({
        ...
        subject: `Pago confirmado — ${info.payer?.first_name || "paciente"} — ${info.transaction_amount} ${info.currency_id}`,
        html: `
